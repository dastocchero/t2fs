...
HANDLER2 h_files[MAX_HANDLERS];

HANDLER2 h_dir[MAX_HANDLERS];

char *current_wd;


unsigned int getSectorByInode(DWORD inodeNumber){

}

unsigned int SectorInodeID(DWORD inodeNumber){
}
...
int identify2 (char *name, int size){
    char group[150] = "Diego Alvim Stocchero - 00260844\nGuilherme Fonseca Ribeiro - 00228415\o"
    if (strlen(group)>=size){
        strcpy(name, group);
        return SUCCESS;
    }
...
FILE2 create2 (char *filename){
    if (FS_initialize()== SUCCESS){
        return FS_create(filename, TYPEVAL_REGULAR);
    }
    return ERROR;
...
int delete2 (char *filename){
    if (FS_initialized()== SUCCESS){
        return FS_delete(filename, TYPEVAL_REGULAR);
    }
    return ERROR;
